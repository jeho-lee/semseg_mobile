...
//    var detectedScore: MutableList<Float> = mutableListOf<Float>(),
    var predictedPixels: Array<LongArray> = arrayOf(longArrayOf()),
...
internal class ORTAnalyzer(
    ...
    /** MAIN Function of Image Analyzer */
    ...
    override fun analyze(image: ImageProxy) {
        ...
        // DeeplabV3+ semantic segmentation model input
        val rawBitmap = imgBitmap?.let { Bitmap.createScaledBitmap(it, inputWidth, inputHeight, false) }
...
//        val rawBitmap = imgBitmap?.let { Bitmap.createScaledBitmap(it, 224, 224, false) }
        ...
        if (bitmap != null) {
            ...
            // DeeplabV3+ semantic segmentation model input
            val shape = longArrayOf(1, 3, inputWidth.toLong(), inputHeight.toLong())
            ...
            env.use {
                ...
                tensor.use {
                    ...
                    output.use {
                        ...
                        println("[Segmentation Inference time] " + result.processTimeMs.toString() + "ms")


                        println("[Segmentation output]")

                        println("[Output info]")
                        println(output?.get(0)?.info)

                        println("[Output type]")
                        println(output?.get(0)?.type)

                        println("[Output JavaClass]")
                        println(output?.get(0)?.javaClass)


                        @Suppress("UNCHECKED_CAST")
//                        val rawOutput = ((output?.get(0)?.value) as Array<FloatArray>)[0]
                        val outputOnnxValues =
                            output?.get(0)?.value as Array<Array<Array<LongArray>>>

                        /*
                        val pixelArray = outputOnnxValues[0][0]
                        for (x in pixelArray) {
                            println("[$x row]")
                            for (y in x) {
                                print("$y ")
                        ...
                        */
                        ...
                        result.predictedPixels = outputOnnxValues[0][0]
                    }

                    /** Image classification output result usage */
                    /*
                    output.use {
                        result.processTimeMs = SystemClock.uptimeMillis() - startTime
                        @Suppress("UNCHECKED_CAST")
